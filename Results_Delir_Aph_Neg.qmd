...
#| cache: TRUE
p <-       plot_grid(
        res_del_aphasia$p0 + ylim(0, 2500) + theme(legend.position = c(0.83,0.95), 
...
plot_tbl(
    ...
    kable_styling(latex_options = c("scale_down"))
```
The bayesian regression analysis investigates sex differences in the relationship between levels of neglect and the _risk of delirium. Compared to having no neglect, minor, visual, tactile, and multimodal neglect showed high probability of an positive association with occurrence of new delirium with  $P(\beta > 0)$ of >99%, 98.0%, 89.7%, and 99.9%, respectively. (@tbl-delir-neglect2) 
