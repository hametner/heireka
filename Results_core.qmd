...
#| label: fig-age-dist
#| fig-cap: "Distribution of Age by Sex: **(A)** Histogram of and overlaying density estimation of whole cohort, **(B)** Overlay of histogram and density in females and males, **(C)** Overlay of density estimations of females and males."
#| echo: false
#| warning: false
#| error: false
...
The sex distrubtion is consistent across the spectrum of functional dependency prior to stroke (@fig-age-pRS).
...
*Chronological age* and *functional dependency prior stroke (pRS)* are therefore most relevant variables to be considerd in further analysis. When comparing earlier with more recent *years*, the number of cases with either intravenous thromobolysis or mechanical thrombectomy or both increased substantially.

#### Operational Aspects

##### Wake-Up Stroke

There are fluctuations in the frequency disparities between biological sexes, with a higher number of female cases seen in some years (e.g., 2011, 2017, and 2018), while in other instances, the reverse tendency emerges (e.g., 2004, 2006, and 2009, [@fig-tele-wakeup B]). Likewise to above mentioned increased since 2015, female wake-up stroke cases have risen considerably compared to males, with the most significant difference in 2018 (@tbl-wakeup-tele).
...
caption <- paste("Variances of predictors of admission National Institutes of Health Stroke Scale Score (NIHSS) based on a bayesian regression model.", s$footnote, getFootnote(bm.nihss.vif))
...
Several aspects are to be considered when NIHSS is used as proxy for stroke severity in statistical models. The following analysis explains why consideration of stroke location when introducing NIHSS as a measure of stroke severity is rather important. Leaving aside cases who have been classified as posterior circulation stroke (n = `r df$side_stroke[df$side_stroke=="posterior"] %>% length`, `r (df$side_stroke[df$side_stroke=="posterior"] %>% length / df$side_stroke %>% length * 100) %>% round(1)`%), and bilateral strokes (n = `r df$side_stroke[df$side_stroke=="bilateral"] %>% length`, `r (df$side_stroke[df$side_stroke=="bilateral"] %>% length / df$side_stroke %>% length * 100) %>% round(1)`%, @tbl-stroke-side), the remaining cases (n = `r dfx %>% nrow()`) are facilitated for the analysis of this chapter.
