...
tar_load(data_dic)
d <- data.frame(var = bm.delir$Design$label)
bm.delir$Design$label <- updateLabels(d, data_dic, rows=TRUE, update_rows_of_col = "var") %>% pull(var)
n <- nomogram(bm.delir, fun=plogis, fun.at=c(.001,.01,.05,seq(.1,.9,by=.1),.95,.99,.999), lp=FALSE)
```

```{r fig-delirium-nomogram, fig.width=10.5, fig.asp=1.3, out.width="100%"}
