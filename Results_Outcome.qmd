...
#| warning: false
#| error: false
#| fig-cap: !expr 'paste0("**(A)** Comparison of *Barthel Index categories* between females and males. Delta values (absolute) for each category are given. **(B)** Probability of a higher Barthel score at discharge as a function of age")'
#| cache: TRUE
...
p1 <- ggplot(Predict(bm_nihss_dis_ia, age, var_microangio, sex, funint=FALSE, fun=plogis, conf.int=FALSE, np=1000), adj.subtitle = 0) + theme_ch() + coord_cartesian(ylim=c(0, 1)) +  theme(
        ...
        plot.title = element_text(size = 10, hjust = 0.5),
        legend.background = element_rect(fill = NA),
        legend.key.size = unit(0.2, "cm")) + ggtitle("Small Vessel disease")+ ylab("Prob. of higher NIHSS score at discharge")
...
#| warning: false
#| error: false
#| fig-cap: !expr 'paste0("Illustration of sex moderating predisposing factors, adverse outcomes, and selected pre-stroke medication, when predicting _NIHSS score at discharge_. (**A**) Presence of small vessel disease (**B**) New sepsis (**C**) New delirium (**D**) Ibuprofen (**E**) Pravastatin (**F**) Humaninsulin")'
#| cache: TRUE
p
```
...
Investigating in-hospital death showed that when a diagnosis of sepsis is made patients had a nearly 22-fold increased risk of dying (`r getMedCI(summary(bm_outcome_ihd), var="outcome_sepsis")`. Comparing females to males as independent predictor, females had significantly lower risk of death in hospital (`r getMedCI(summary(bm_outcome_ihd), var="female")`). [^results_outcome-1]
...
```{r fig-ihd-delir, fig.width=4.6, fig.asp=1, out.width="80%"}
#| label: fig-ihd-delir
#| echo: false
#| warning: false
#| error: false
...
At the end of the hospital stay, a new adverse outcome (e.g. delirium) may have occurred, a new diagnosis regarding stroke etiology has been made, and there may have already been a shift in recovery. This analysis focuses on the factors identified at hospital discharge and how they relate to long-term functional outcomes, as measured by the Modified Rankin Scale at 90 days. It specifically emphasizes the impact of delirium and biological sex, among other factors.
...
As for sex differences in adverse events, pneumonia showed no meaningful sex difference after adjustment (`r getP(getRsmb(bm_outcome_mRSd90_ia), var="sex=male * outcome_pneumonia=1")`). Sepsis, myocardial infarction, and deep vein thrombosis were all with a beneficial effect towards females 
( $P_{Interaction}(\beta>0)$: sepsis: `r getP(getRsmb(bm_outcome_mRSd90_ia), var="sex=male * outcome_sepsis=1")`, myocardial infarction: `r getP(getRsmb(bm_outcome_mRSd90_ia), var="sex=male * outcome_mi=1")`, deep vein thrombosis: `r getP(getRsmb(bm_outcome_mRSd90_ia), var="sex=male * outcome_tvt=1")`, @fig-mrsd90-ia A--B illustrates this pattern with the example of sepsis).
...
library(gt)
library(kableExtra)
t_mrsd90 <- getLatexTableCH(bm_outcome_mrsd90, font_size=8.5, output_format="latex", option="longtable") %>% 
         column_spec(1, "1.9in") %>% 
        column_spec(2, "0.3in") %>%
        column_spec(3, "0.3in") %>%
        column_spec(4, "0.3in") %>%
...
p1 <- plotInteraction(bm_outcome_mRSd90_ia,
                ...
                group_var = "sex", display_annotations=FALSE) + theme_ch() + ylab("P(mRS-shift|data,x)") + ylim(0,1)+
        theme(
        legend.position = "top",
        plot.title = element_text(size = 10, hjust = 0.5),
        legend.background = element_rect(fill = NA),
...
p2 <- ggplot(Predict(bm_outcome_mRSd90_ia, age, outcome_sepsis, sex, conf.int = FALSE, funint=FALSE, fun=plogis), adj.subtitle=0) + theme_ch() +ylab("P(mRS-shift|data,x)") +  coord_cartesian(ylim = c(0, 1)) +
        theme(
        legend.position = "top",
        plot.title = element_text(size = 10, hjust = 0.5),
        legend.background = element_rect(fill = NA),
...
p3 <- plotInteraction(bm_outcome_mRSd90_ia,
                ...
                group_var = "sex", display_annotations=FALSE) + theme_ch() + ylab("P(mRS-shift|data,x)") + ylim(0.7,1)+
        theme(
        legend.position = "top",
        plot.title = element_text(size = 10, hjust = 0.5),
        legend.background = element_rect(fill = NA),
...
p4 <- ggplot(Predict(bm_outcome_mRSd90_ia, date_therapy_year, therapy, sex, conf.int = FALSE, funint=FALSE, fun=plogis), adj.subtitle=0) + theme_ch() +ylab("P(mRS-shift|data,x)") + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + coord_cartesian(ylim = c(0.7, 1)) +
        theme(
        legend.position = "top",
        plot.title = element_text(size = 10, hjust = 0.5),
        legend.background = element_rect(fill = NA),
...
p6 <- ggplot(Predict(bm_outcome_mRSd90_ia, outcome_delir_lohs_days, outcome_delir, sex, conf.int = FALSE, funint=FALSE, fun=plogis), adj.subtitle=0) + theme_ch() +ylab("P(mRS-shift|data,x)") + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + coord_cartesian(ylim = c(0.7, 1)) +
        theme(
        legend.position = "top",
        plot.title = element_text(size = 10, hjust = 0.5),
        legend.background = element_rect(fill = NA),
...
#| warning: false
#| error: false
#| fig-cap: !expr 'paste0("Illustration of sex moderating , when predicting a shift towards a higher category on the _modified Rankin Scale d90_")'
#| cache: TRUE
p_mrsd90
```
...
library(gt)
library(kableExtra)
t_mrs02 <- getLatexTableCH(bm_outcome_mrs02, font_size=8.5, output_format="latex", option="longtable") %>% 
        column_spec(1, "1.9in") %>% 
        column_spec(2, "0.3in") %>%
        column_spec(3, "0.3in") %>%
        column_spec(4, "0.3in") %>%
